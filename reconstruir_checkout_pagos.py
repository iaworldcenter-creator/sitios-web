    ...
    <main class="max-w-7xl mx-auto px-4 py-8 flex-1 w-full">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div class="lg:col-span-8 flex flex-col gap-6">
                ...
                <!-- PASO 2: MÉTODO DE PAGO MEJORADO -->
                <div class="bg-slate-900/90 border border-slate-800 rounded-2xl p-6 shadow-xl relative">
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center gap-3">
                    ...
                    <div id="step-2-form" class="flex flex-col gap-4">
                        ...
                        <!-- PANEL 2: TARJETA DE CRÉDITO/DÉBITO -->
                        <div id="panel-tarjeta" class="hidden p-4 bg-slate-950 border border-cyan-500/40 rounded-xl flex flex-col gap-3">
                            <span class="text-xs font-bold text-cyan-400 flex items-center gap-1.5 border-b border-slate-800 pb-2">
                                <i class="fa-solid fa-credit-card"></i> Ingresa los datos de tu Tarjeta
                            </span>
...
def build_all_checkouts():
    print("=== RECONSTRUYENDO CHECKOUT CON PASARELA DE PAGOS Y CLABE ===")
    for store_slug, store_title in STORES.items():
        ...
        print(f"[OK] Checkout y Pasarela listos en: {store_slug}/checkout.html")
    ...
    for store_slug in STORES.keys():
        ...
        if os.path.exists(os.path.join(store_dir, ".git")):
            ...
            subprocess.run(["git", "commit", "-m", "fix(payments): pasarela SPEI con CLABE 012180015425843949, tarjeta y oxxo", "--allow-empty"], cwd=store_dir, capture_output=True)
            subprocess.run(["git", "-c", "gc.auto=0", "push", "origin", "main"], cwd=store_dir, capture_output=True)
            print(f"🟢 Push OK: {store_slug}")

    subprocess.run(["git", "add", "-A"], cwd=BASE_DIR, check=True)
    subprocess.run(["git", "commit", "-m", "feat(payments): integracion completa de metodos de pago y clabe en checkout", "--allow-empty"], cwd=BASE_DIR, capture_output=True)

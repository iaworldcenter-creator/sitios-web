...
print("=" * 75)
print("CORRIGIENDO ENLACES Y ERRORES 404 EN EL PORTAL MATRIZ (7 BOUTIQUES + MATRIZ)")
print("=" * 75)

PORTAL_CORREGIDO_HTML = """<!DOCTYPE html>
...
<body class="bg-slate-950 text-slate-100 font-sans antialiased min-h-screen flex flex-col justify-between overflow-x-hidden selection:bg-cyan-500 selection:text-slate-950">
    ...
    <!-- TOP BAR DE NAVEGACIÓN CRUZADA CON ENLACES CORREGIDOS -->
    <header class="bg-slate-900/95 backdrop-blur border-b border-slate-800 sticky top-0 z-50 shadow-xl">
        <div class="max-w-7xl mx-auto px-4 py-2.5 flex flex-wrap items-center justify-between gap-4">
            ...
            <!-- Accesos directos a las 7 Boutiques + Portal Central Autorrefrescable -->
    ...
    <!-- RETÍCULA OFICIAL DE 7 BOUTIQUES (SIN DUPLICADOS) -->
    <main class="max-w-7xl mx-auto px-4 py-12 flex-1 w-full">
        <div class="flex flex-col gap-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div class="bg-slate-900/90 border border-cyan-500/40 hover:border-cyan-400 rounded-2xl p-5 shadow-xl shadow-cyan-950/20 transition-all duration-300 hover:-translate-y-1 flex flex-col justify-between group relative overflow-hidden">
                    ...
                    <div class="flex flex-col gap-3">
                        ...
                        <p class="text-xs text-slate-400 leading-relaxed">
                            Catálogo selecto de 200 artículos en 30 departamentos: electrónica de consumo, electrodomésticos, smart home y oportunidades de importación.
                        </p>
                    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto w-full">
                ...
                <!-- 7. OFERTAS & LIQUIDACIONES (CON RUTA VERIFICADA CON GUION FINAL) -->
...
for path in portal_paths:
    if os.path.exists(os.path.dirname(path)) or os.path.exists(path):
        with open(path, "w", encoding="utf-8") as f:
            f.write(PORTAL_CORREGIDO_HTML)
        print(f"✓ Portal Matriz corregido en: {path}")
...
if os.path.exists(os.path.join(sitios_web_repo, ".git")):
    ...
    subprocess.run(["git", "commit", "-m", "fix(links): corregir 404 liquidaciones, normalizar casing Vía MX y autorrefresco matriz", "--allow-empty"], cwd=sitios_web_repo, capture_output=True)
...
subprocess.run(["git", "commit", "-m", "fix(ecosistema): enlaces corregidos sin 404 ni duplicados desplegados en GitHub", "--allow-empty"], cwd=BASE_DIR, capture_output=True)

...
print("=" * 80, flush=True)
print("APLICACIÓN RIGUROSA DE LA GUÍA MAESTRA DE OPTIMIZACIÓN (WCAG 2.2 / CLS ZERO)")
print("=" * 80, flush=True)

# 1. LEER PRODUCTOS INICIALES PARA SSR PRE-RENDER
...
# 2. DEFINICIÓN DE ESTILOS CRÍTICOS CON REGLAS EXPLÍCITAS DE CLS Y ACCESIBILIDAD
CRITICAL_CSS_MASTER = """
/* === GUÍA MAESTRA: ESTILOS CRÍTICOS CON CONTENCIÓN DE LAYOUT Y ANULACIONES MÉTRICAS === */
.product-img-wrapper {
...
/* Áreas de contacto calibradas >= 48px (Cumplimiento estricto WCAG 2.2 / WCAG 2.5.8) */
...
/* Anulaciones métricas de fuentes tipográficas para cero saltos FOUT / CLS */
@font-face {
...
with open(os.path.join(BASE_DIR, "assets", "css", "tailwind-built.css"), "r", encoding="utf-8") as f:
    tailwind_base = f.read()

FINAL_CSS = tailwind_base + "\n" + CRITICAL_CSS_MASTER

# 3. CONSTRUCCIÓN DE INDEX.HTML
...
{ssr_grid_content}
    ...
    <script src="js/ct-catalog-data.js?v=20260828_2150"></script>
    <script defer src="js/ct-exact-catalog-engine.js?v=20260828_2150"></script>
</body>
</html>
"""
...
print("✅ index.html reescrito con reglas explícitas de CLS < 0.05, SSR pre-render y WCAG 2.2.", flush=True)

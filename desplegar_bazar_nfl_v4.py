...
<body class="bg-slate-950 text-slate-100 font-sans antialiased min-h-screen flex flex-col justify-between overflow-x-hidden selection:bg-cyan-500 selection:text-slate-950">
            ...
            <div class="flex-1 max-w-3xl w-full relative">
                <div class="flex items-center bg-white rounded-full border-2 border-cyan-400 shadow-[0_0_22px_rgba(6,182,212,0.4)] px-4 py-1.5 gap-2">
                    ...
                    <input 
                        ...
                        autocomplete="off"
                        spellcheck="false"
                        placeholder="Busca, encuentra y compra rápido (ej. RAM Kingston, Marlboro, Paletas, Cohiba, Lentes)..." 
    ...
    <!-- ========================================================================
         4. CART DRAWER LATERAL CON STEPPER Y PROGRESO DE ENVÍO GRATIS
         ======================================================================== -->
            ...
            <div>
                <div class="flex justify-between items-center border-b border-slate-800 pb-3 mb-4">
                    <h3 class="font-black text-white text-base flex items-center gap-2">
                        <i class="fa-solid fa-cart-shopping text-cyan-400"></i> Canasta BAZAR NFL.GDL
                    </h3>
    ...
    <footer class="bg-slate-950 border-t border-slate-900 pt-16 pb-8 text-slate-400 text-xs" id="pie-de-pagina">
        <div class="max-w-[99%] 2xl:max-w-[1850px] mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="pt-8 text-center text-slate-500 text-[11px]">
                <p>&copy; 2026 BAZAR NFL.GDL & Ecosistema Comercial Pedro Moreno 501 A. Guadalajara Centro. Todos los derechos reservados.</p>
            </div>
        </div>
    </footer>
    ...
    const masterItems = [
        ...
        { sku: "OFE-004", boutiqueId: "ofertas", nombre: "Bocina Bluetooth Portátil contra Agua 20W Excedente de Stock", marca: "JBL", precio: 750.00, original: 1100.00, desc: "Sonido de alta fidelidad con batería de 12 horas de duración continua.", img: "https://iaworldcenter-creator.github.io/pc-custom-lab/assets/img/slider_warehouse_thumb.webp", tokens: ["bocina", "jbl", "bluetooth", "audio", "remate"] },
        { sku: "OFE-005", boutiqueId: "ofertas", nombre: "Caja Sorpresa de Novedades y Gadgets Mixtos (10 Piezas)", marca: "Bazar NFL.GDL", precio: 999.00, original: 1800.00, desc: "Artículos variados de conveniencia garantizados con valor superior al precio pagado.", img: "https://iaworldcenter-creator.github.io/pc-custom-lab/assets/img/mascota_tigre.webp", tokens: ["caja sorpresa", "mistery box", "novedades", "lote"] }
    ];
    ...
    function renderSidebarBoutiques() {
        ...
        container.innerHTML = boutiquesConfig.map(b => `
            <div class="sidebar-boutique-row" data-id="${b.id}">
                <button 
    ...
    function handleSidebarHover(boutiqueId, element) {
        ...
        const tag = document.getElementById("flyout-boutique-tag");
        const title = document.getElementById("flyout-boutique-title");
        const list = document.getElementById("flyout-boutique-products");
    ...
    const sidebarRoot = document.getElementById("portal-sidebar-root");
    sidebarRoot.addEventListener("mouseleave", () => {
        flyoutTimeout = setTimeout(() => {
            document.getElementById("sidebar-flyout-panel").classList.add("hidden");
        }, 300);
    });
    ...
    // ------------------------------------------------------------------------
    // MOTOR DE BÚSQUEDA MULTI-TOKEN CON COMPRA DIRECTA EN EL DESPLEGABLE
    // ------------------------------------------------------------------------
        ...
        if (matches.length === 0) {
            box.innerHTML = `
                <div class="p-3 text-center text-slate-400 text-xs">
                    <span class="block text-white font-bold mb-1">No hay coincidencias exactas para "${val}"</span>
                    <span class="text-[11px] text-slate-500">Prueba con palabras clave como RAM, Marlboro, Paletas, Cohiba, Lentes o revisa las secciones abajo.</span>
                </div>
            `;
    ...
    // ------------------------------------------------------------------------
    // GESTIÓN DEL CARRITO GLOBAL CON BARRA DE PROGRESO DE ENVÍO GRATIS ($1,500)
    // ------------------------------------------------------------------------
    ...
    function syncCartState() {
        ...
        document.getElementById("portal-cart-badge").innerText = totalCount;
        document.getElementById("portal-cart-total").innerText = `$${totalMoney.toLocaleString('es-MX', { minimumFractionDigits: 2 })} MXN`;
        document.getElementById("drawer-total-txt").innerText = `$${totalMoney.toLocaleString('es-MX', { minimumFractionDigits: 2 })} MXN`;

        // Cálculo de envío gratis ($1,500 MXN)

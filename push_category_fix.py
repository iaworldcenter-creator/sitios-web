...
print("Staging and pushing mobile category navigation in pc-custom-lab...")
subprocess.run(["git", "add", "-A"], cwd=PC_DIR)
subprocess.run(["git", "commit", "-m", "fix(nav): navegacion por categorias reactiva con auto-scroll a la vitrina en movil y desktop con IDs sincronizados"], cwd=PC_DIR)
...
subprocess.run(["git", "commit", "-m", "fix(ecosistema): sincronizacion de navegacion por departamentos con auto-scroll"], cwd=BASE_DIR)
